...
import { WHATSAPP_NUMBER, WhatsAppIcon, whatsappLink } from "@/components/ausvape/WhatsApp";
...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      { title: "Contact AUSVAPE CO — WhatsApp Support & Melbourne Counter" },
      { name: "description", content: "Message the AUSVAPE CO team on WhatsApp for order, shipping and returns help, or visit our Melbourne CBD counter Mon–Fri 10am–5pm." },
      { property: "og:title", content: "Contact AUSVAPE CO — WhatsApp Support & Melbourne Counter" },
      { property: "og:description", content: "WhatsApp support for orders, shipping and returns, plus our Melbourne CBD pickup counter." },
    ...
    <SiteLayout>
      <section className="px-4 md:px-8 pt-20 pb-8 border-b border-[#A9791F]/15">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="mt-4 text-[color:var(--color-smoke)]">We reply on WhatsApp, usually within the hour.</p>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto grid lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 bg-[#18181B] border border-[#A9791F]/15 rounded-lg p-8">
            {sent && (
              <div role="status" className="mb-6 flex items-start gap-3 rounded border border-[#A9791F]/30 bg-[#0A0A0C] p-4 text-sm">
                ...
                <span>
                  Your message is ready in WhatsApp. Hit send and we&apos;ll reply shortly —{" "}
                  <a href={whatsappLink()} target="_blank" rel="noopener noreferrer" className="text-gold">open the chat</a>.
                </span>
              </div>
            )}
            <form className="grid sm:grid-cols-2 gap-4" onSubmit={handleSubmit} noValidate>
              ...
              <button className="sm:col-span-2 inline-flex items-center justify-center gap-2 bg-gold text-[#0A0A0C] font-semibold py-3 rounded hover:shadow-[0_0_24px_rgba(240,205,110,0.4)] transition-shadow">
          ...
          <aside className="space-y-6">
            <Card
              ...
              title="WhatsApp"
              body={
                <a href={whatsappLink()} target="_blank" rel="noopener noreferrer" aria-label="Chat with AUSVAPE CO on WhatsApp" className="text-gold">
                  {WHATSAPP_NUMBER}
                </a>
              }
            />
            <Card icon={<Clock className="w-4 h-4" aria-hidden="true" />} title="Support Hours" body={<span className="text-[color:var(--color-smoke)]">Mon–Fri 10am–5pm AEST<br />WhatsApp replies within the hour</span>} />
