...
export const Route = createFileRoute("/blog/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Guides and notes from AUSVAPE CO: verifying authentic IGET vapes, spotting counterfeit pods, and how we curate our Melbourne range." },
...
const posts = [
  {
    title: "IGET vape authenticity & verification guide",
    excerpt: "Batch codes, scratch panels and the six tells that separate a genuine IGET from a fake.",
    img: p2.url,
    date: "4 August 2026",
    tag: "Guides",
    to: "/blog/iget-authenticity-guide" as const,
  },
    ...
    <SiteLayout>
      ...
      <section className="px-4 md:px-8 py-16">
        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-6">
          {posts.map(p => {
            const card = (
              <article className="group h-full bg-[#18181B] border border-[#A9791F]/15 rounded-lg overflow-hidden hover:border-[#A9791F]/50 transition-colors">
            ...
            return p.to ? (
              <Link key={p.title} to={p.to} className="block h-full">{card}</Link>
            ) : (
